...
This experimental dev build uses a concept-first extraction workflow for broad page requests. After the brand kit, Maquette generates separate 16:9 desktop browser screenshot concepts for the top, middle, and bottom page segments and asks whether to use each one. It does not use one compressed full-page concept for implementation. From the approved segments it writes `.maquette/pages/<page-name>/component-extraction-plan.md`, updates `.maquette/components/component-coverage-plan.md`, then creates only the reusable component coverage needed to clone those segments. Component coverage now defaults to focused 1:1 visual component close-ups edited from the approved concept segments, optionally using the bundled neutral close-up template as a scaffold. Codex inspects one close-up at a time, writes reusable HTML/CSS/JS, captures a rendered component screenshot no larger than 1254x1254, compares it to the close-up, fixes mismatches, and only then moves to the next component. Legacy CSS text-on-image posters are explicit-only experiments, not the default route.
...
- page concept phase -> create or edit separate top, middle, and bottom 16:9 **desktop browser screenshot concept images** for discovery and implementation fidelity; do not use one compressed full-page concept for implementation
- components phase -> create `.maquette/pages/<page-name>/component-extraction-plan.md`, update a dynamic **component coverage plan**, then create one focused 1:1 **visual component close-up** at a time from the approved concept segments for missing reusable coverage justified by the coverage plan. Use CSS text-on-image posters only when explicitly requested as the legacy route.
...
Maquette includes optional neutral template images under `shared/template-images/`: `brand-kit-template-v1.png` for structured brand boards and `component-closeup-template-v1.png` for focused component close-ups. Use them as edit-mode scaffolds for framing and readability, not as design authority. If a template makes an artifact generic, crowded, or less faithful to the brief or concept, bypass it and document why.
...
Component implementation includes hard gates per artifact: first make the optional QA tooling decision, then write the coverage plan, create one focused 1:1 visual component close-up from the approved concept for a documented reusable gap, render an artifact-specific componentized replica/reference using reusable CSS/JS, screenshot-match it with evidence capped at 1254x1254, write batch artifacts, and only then move to the next artifact. Every close-up should usually cover one major component family, or at most two tightly related small families, and must remain inspectable at normal preview size.
Page implementation includes fidelity and UX gates: inventory visible concept regions, write segment layout contracts for top/middle/bottom viewport-scale fidelity, typography, text fit, and overflow, write a page layout contract for section compactness, image fit/crop behavior, terminal regions, and responsive structure, create an asset manifest for required raster assets, write an experience quality contract for generated visual fit, motion/effects, states, accessibility, performance, information architecture, brand craft, and mobile UX, then document section-by-section screenshot comparison notes before approval. Top, middle, and bottom segments must pass visual QA before Maquette assembles or approves the final page.
...
The component extraction plan is the page-specific bridge from approved concept segments to reusable components.
...
If the approved brand system uses custom, imported, hosted, or non-system fonts, Maquette should create `.maquette/brand/fonts.css`; every generated HTML artifact should import that file before `tokens.css`, component CSS, or page CSS.
...
In concept-first mode, this pass creates separate 16:9 desktop top, middle, and bottom page concept segments, asks for approval on each, and writes the component extraction plan before component expansion. In implementation mode, it updates the component coverage plan for the page, writes segment layout contracts and a page layout contract for section density, media crops, terminal sections, typography, text fit, overflow, and responsive behavior, writes an experience quality contract for generated visual fit, purposeful motion/effects, reduced-motion behavior, states, accessibility, performance, information architecture, brand craft, and mobile UX, implements the page with the approved brand and component references, captures segmented screenshots capped at 1254x1254 when possible, and records review notes.
...
node plugins/maquette/shared/scripts/capture-browser.mjs .maquette/components/replica-gallery.html .maquette/components/replica-gallery.png --width 1254 --height 1254 --json .maquette/components/reference-capture.json
node plugins/maquette/skills/maquette-components/scripts/capture-gallery.mjs .maquette/components/replica-gallery.html .maquette/components/replica-gallery.png --width 1254 --height 1254
node plugins/maquette/skills/maquette-pages/scripts/capture-page.mjs .maquette/pages/homepage/page.html .maquette/pages/homepage/screenshots/page-1254.png --mode segments --segments top,middle,bottom --width 1254 --height 1254 --json .maquette/pages/homepage/segment-capture-1254.json
...
node plugins/maquette/shared/scripts/check-text-fit.mjs .maquette/pages/homepage/page.html --width 1365 --height 768 --json .maquette/pages/homepage/text-fit-1365.json
...
Screenshot capture and responsive auditing should stay headless, and every browser instance opened for capture must be closed before the workflow finishes. The bundled scripts close Chromium in a `finally` block. Visual review screenshots should be no larger than 1254x1254. Page visual review should use segmented viewport screenshots, typically top, middle, and bottom/terminal for desktop and mobile, so detailed review happens at normal viewport scale without giant screenshots. Linked asset validation should pass for each batch replica and the final component reference before the next component artifact or page phase begins.
...
Desktop fidelity review should also capture concept-ratio screenshots and common desktop widths, including Windows-like browser widths such as 1365, 1366, 1440, 1536, and 1600px when tooling allows. Run `shared/scripts/check-text-fit.mjs` when browser tooling is available to fail nav, button, placeholder, card, chip, table, metric-label, footer-link, font-loading, and horizontal-overflow regressions.
