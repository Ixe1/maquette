...
The components phase defaults to focused direct CSS-contract posters for this experiment. Before generating any poster, Maquette writes `.maquette/components/component-coverage-plan.md` by inspecting the brief, requested pages, existing component catalog, existing contract CSS, component CSS/JS, and replica/gallery examples. It reuses existing components first, extends them when only a variant, state, slot, density, or behavior is missing, and creates new reusable components only for true structural gaps. Then Maquette generates a focused 1:1 black-background CSS-contract poster for one justified component family, inspects it, transcribes the recovered rules into a reviewable `.maquette/components/contracts/<batch>.contract.css` bridge file, implements the reusable component batch, and uses browser screenshots as the visual correction target. Visual component sheets are fallback, supplement, or explicit-request artifacts.
...
- components phase -> create or update a dynamic **component coverage plan**, then create one focused 1:1 **CSS-contract poster** at a time for missing reusable coverage justified by that plan. Create visual component sheet images only when explicitly requested, when a CSS-contract poster is too generic to guide implementation, or when a focused visual supplement is needed for fidelity.
...
Component implementation includes hard gates per artifact: first make the optional QA tooling decision, then write the coverage plan, generate one 1:1 CSS-contract poster by default for a documented reusable gap, render an artifact-specific componentized replica/reference using reusable CSS/JS, write batch artifacts, and only then move to the next artifact. Every poster must use a strict selector allowlist, should usually contain roughly 8 to 16 meaningful selectors, and must remain readable at normal preview size.
...
The component coverage plan is the Codex-readable planning artifact for reuse, extension, true reusable gaps, page-specific composites, and focused poster batches.
The CSS-contract poster is the default 1:1 text source for selectors, states, slots, dimensions, motion notes, responsive notes, accessibility notes, and performance-safe property guidance, not a full visual design target. The transcribed contract CSS is the implementation bridge from inspected image text to final tokenized component CSS.
...
New pages consume the existing component catalog first. A new component poster is appropriate only when the coverage plan proves existing APIs cannot cleanly cover the need.
