...
The components phase defaults to focused CSS text-on-image component sheets again. Before generating any poster, Maquette writes `.maquette/components/component-coverage-plan.md` by inspecting the brief, requested pages, existing component catalog, existing contract CSS, component CSS/JS, and replica/gallery examples. It reuses existing components first, extends them when only a variant, state, slot, density, or behavior is missing, and creates new reusable components only for true structural gaps. Then Maquette generates a focused 1:1 CSS-contract poster for one justified component family, inspects it, transcribes it into `.maquette/components/contracts/<batch-slug>.contract.css`, implements the reusable component batch, and uses browser screenshots as the visual correction target. Visual component sheets are optional supplements when the user asks for one or when the poster needs anatomy/fidelity clarification.
...
- components phase -> create or update a dynamic **component coverage plan**, then create one focused 1:1 **CSS-contract component sheet poster** at a time for missing reusable coverage justified by that plan. Create visual component sheets only when explicitly requested or when a poster needs anatomy/fidelity clarification.
...
Generated boards and sheets should be readable at normal preview size. Maquette should regenerate, edit, or split component posters that are cluttered, unreadable, selector-bloated, logo-like, or not inspectable enough to guide implementation.
...
Component implementation includes hard gates per artifact: first make the optional QA tooling decision, then write the coverage plan, generate one focused 1:1 CSS-contract poster by default for a documented reusable gap, transcribe it, render an artifact-specific componentized replica/reference using reusable CSS/JS, write batch artifacts, and only then move to the next artifact. Every poster should usually cover one major component family, or at most two tightly related small families, and must remain readable at normal preview size with a strict selector allowlist.
...
The component coverage plan is the Codex-readable planning artifact for reuse, extension, true reusable gaps, page-specific composites, and focused CSS-contract poster batches.
The focused CSS-contract poster is the default 1:1 component artifact for selectors, slots, variants, states, density, spacing, radius, shadows, motion/effect cues, responsive behavior, accessibility hooks, performance-safe property guidance, and reusable patterns.
An optional visual component sheet is a supplement for anatomy, density, spacing, hierarchy, and polish when the poster needs visual clarification. The transcribed contract CSS is the implementation bridge from inspected image text to final tokenized component CSS for every default poster.
...
This pass first creates or updates `.maquette/components/component-coverage-plan.md`. The plan is dynamic per project: it records current required families, likely future reusable families to avoid overbuilding, existing coverage, extension-only needs, true reusable gaps, page-specific composites, focused CSS-contract poster batches, intended component API targets, strict selector allowlists, and rationale. It should not use a fixed universal component taxonomy.
After the plan identifies a needed reusable gap, Maquette creates a focused CSS-contract poster, transcribes it into a contract CSS bridge, builds and reviews a componentized visual reference with reusable classes, states, slots, and usage examples, writes batch artifacts, then repeats that loop only for additional focused artifacts the product actually needs. Visual component sheets can be generated as optional supplements when a poster needs anatomy/fidelity clarification.
...
New pages consume the existing component catalog first. A new CSS-contract poster is appropriate only when the coverage plan proves existing APIs cannot cleanly cover the need.
