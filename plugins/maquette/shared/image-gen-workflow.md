...
2. Components
   - create or update `.maquette/components/component-coverage-plan.md`, then generate one focused 1:1 CSS-contract component sheet poster at a time by default for reusable gaps justified by that plan
   - generate a focused 1:1 visual component sheet only when the user explicitly asks for one or when the component skill documents that a visual anatomy/fidelity supplement is needed
3. Pages
...
Only after the generated artifact, and any optional visual supplement when needed, exists should the workflow proceed to code implementation.
...
- Before any CSS-contract poster or optional visual component sheet is generated, `.maquette/components/component-coverage-plan.md` must document the project-specific need, existing component reuse check, extension decisions, reusable gaps, page-specific composites, intended API targets, strict selector allowlist, and why each new poster or optional sheet is needed.
- CSS-contract posters are the default component design artifacts. They are focused text-on-image component sheets for selectors, states, slots, dimensions, token intent, motion notes, responsive notes, accessibility notes, and performance-safe property guidance. They must use 1:1 square composition, black or near-black background, readable white monospace CSS-like text, and a strict selector allowlist from the coverage plan. They should usually cover one major component family, or at most two tightly related small families, with roughly 8 to 16 meaningful selectors. They must not include `body`, `html`, reset, page layout, gallery, panel, note, or documentation CSS.
- Visual component sheets are optional fidelity supplements. Use them when the user asks for one or when a CSS-contract poster is too abstract for reliable visual anatomy, density, spacing, hierarchy, or polish. They must use 1:1 square composition and be split into focused 1:1 sheets when a single sheet would become cluttered or uninspectable.
...
- Each CSS-contract poster or optional visual component sheet batch must complete screenshot review or documented manual visual review against the generated artifact before the next artifact is generated.
- CSS-contract posters are the default source of truth for component families, selectors, variants, states, anatomy, density, spacing, radius, shadows, polish, motion/effects, responsive notes, and composites while using reusable CSS/JS and cataloged APIs from the start. If an optional visual component sheet is used, coded componentized references must also honor its visible anatomy, density, spacing, and polish.
...
Before component coding, write a component coverage plan and sheet inventory. The coverage plan documents reuse/extend/create/page-specific decisions, intended API targets, and required selector allowlists before image generation. The sheet inventory lists contracted or visible component families, variants, states, larger patterns, unreadable/unclear/cramped areas, missing coverage, and the decision to implement, regenerate, split, or create another focused poster or optional visual sheet.

Before accepting component implementation, compare the coded componentized reference screenshots against the approved focused CSS-contract posters, and optional visual component sheets when used, with the component fidelity rubric:
- coverage: contracted or visible component families, variants, and states are implemented
- visual match: match the focused CSS-contract poster and approved brand closely enough after screenshot review; also match the optional visual sheet when one was used
