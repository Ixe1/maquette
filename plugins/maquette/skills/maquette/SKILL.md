...
For broad page or site requests in this experimental dev build, run the workflow in this order:
...
Before creating a page implementation, verify that these component artifacts exist:
...
- `.maquette/components/sheet-implementation-log.md` when multiple component artifacts were needed
...
- one or more generated and inspected visual component close-up artifacts, such as `.maquette/components/component-closeup-*-vN.png`
- `.maquette/components/contracts/<batch-slug>.contract.css` only for each explicit legacy CSS-contract poster batch

If any are missing, use the approved page concept and component extraction plan to run the component-library phase next using `maquette-components`.
...
If the requested page needs dense data patterns, dashboards, tables, maps, calendars, editors, timelines, complex workflows, filter builders, or reusable composites, first update `.maquette/components/component-coverage-plan.md` by inspecting the component catalog, contracts when present, component CSS/JS, and replica/gallery examples. Reuse or extend existing components when they cover the need. Run `maquette-components` again only when the updated plan identifies a true reusable coverage gap and explains why a new focused component close-up is needed. Multi-artifact component work should proceed sequentially: generate one focused 1:1 visual close-up by default, or one focused 1:1 legacy CSS-contract poster only when explicitly requested, build and review its componentized replica/reference, document reusable component APIs, then move to the next artifact.
